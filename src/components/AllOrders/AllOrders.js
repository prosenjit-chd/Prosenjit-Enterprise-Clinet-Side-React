...
const AllOrders = () => {
    ...
    useEffect(() => {
        ...
        axios.get('https://prosenjit-enterprise-server-side-node.onrender.com/api/allorders', header)
    ...
    const handleUserStatus = (id) => {
        ...
        axios.patch(`https://prosenjit-enterprise-server-side-node.onrender.com/api/orders/${id}`, updateUser)
            .then(res => {
                if (res > 0) {
                    swal({
                        title: "Thank you Sir",
